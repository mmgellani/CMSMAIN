...
namespace Cms360.UI.Controllers.Account
{
    ...
    public class ExaminationGradingDetailController : Controller
    {
        private readonly IExaminationGradingDetailRepository repository;

        private readonly DbContextBase db;

        public ExaminationGradingDetailController(IExaminationGradingDetailRepository repository, DbContextBase db)
        {
        ...
        [HttpPost]
        [Route("[action]")]
        public IActionResult GetFind([FromBody] Predicate predicate)
        {
            var gradingMasterId = new Guid(predicate.ProvidedString.Split("?")[0]);
        
            return Ok(this.db.ExaminationGradingDetail.Where(e => e.StatusId == 1 && e.GradingMasterId == gradingMasterId));
        }
    ...
    //     }
        ...
        [HttpPost]
        [Route("[action]")]
        public IActionResult DeleteGradingDetail([FromBody]Predicate entity)
        {
             var list= (entity.ProvidedString);
           

            string json = String.Format("SELECT \"Examination\".\"DeleteGradingdetail\"('{0}')", list);
