...
using Microsoft.AspNetCore.DataProtection.StackExchangeRedis; //ADDED FOR KEYS BY MUZZAMMIL
using System.IO;
...
namespace Cms360.Server
{
    public static partial class Extensions
    {
        ...
        public static IConfigurationBuilder AddCms360(this IConfigurationBuilder builder)
        {
        ...
        public static void ConfigureMvc(this IServiceCollection services, Config.AntiForgeryConfig xsrfConfig)
        {
            ...
            var redis_ip = config.Data.RadisIp;
                StackExchange.Redis.ConnectionMultiplexer connection = StackExchange.Redis.ConnectionMultiplexer.Connect(redis_ip);
                services.AddDataProtection()
                    .PersistKeysToStackExchangeRedis(connection, "ems360-Keys")
                    .SetApplicationName("ems360");

                Console.WriteLine("Connected with Radis IP: "+redis_ip);
            }
            catch(Exception ex)
            {
                Console.WriteLine($"Error: {ex.Message}");
                // services.AddDataProtection()
                // .PersistKeysToFileSystem(new DirectoryInfo("/var/lib/cms/web/keys"))
                // .SetApplicationName("ems360");

                // Console.WriteLine("Connected with Local persistance Key Storage");
            }
