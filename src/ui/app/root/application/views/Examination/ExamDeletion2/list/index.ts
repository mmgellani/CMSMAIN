...
export class ExamDeletion2 extends Vue {
   ...
   get canSave(){
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('examDeletion' in this.user.claims) == true) {
                if (this.user.claims['examDeletion'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['examDeletion'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['examDeletion'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['examDeletion'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if (this.dateMonth.toString().length > 0) {
            if (this.sectionCourseLinkId.length > 0 && this.examTypeId.length > 0) {
                ...
                this.examMasterRepo.GetExamApprovalData(key)
                    .then(response => {
    ...
    insertModel() {
            ...
            this.examMasterRepo.DeleteExamination(this.examasterid)
                .then(r => {
                    this.$store.dispatch(StoreTypes.updateStatusBar, {
