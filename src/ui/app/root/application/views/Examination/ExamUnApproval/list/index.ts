...
export class ExamUnApproval extends Vue {
   ...
   get canSave(){
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('examUnApproval' in this.user.claims) == true) {
                if (this.user.claims['examUnApproval'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['examUnApproval'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['examUnApproval'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['examUnApproval'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if (this.dateMonth.toString().length > 0) {
            if (this.sectionCourseLinkId.length > 0 && this.examTypeId.length > 0) {
                ...
                this.examMasterRepo.GetExamUnApprovalData(key)
                    .then(response => {
    ...
    insertModel() {
        var list = this.data.filter(s => s.isApproved==false)
