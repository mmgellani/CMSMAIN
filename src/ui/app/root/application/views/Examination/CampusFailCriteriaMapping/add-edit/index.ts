...
type ValidateExaminationCampusFailCriteriaMapping = { data: IExaminationCampusFailCriteriaMapping; validationGroup: string[] };
let customValidation: ValidationRuleset<ValidateExaminationCampusFailCriteriaMapping> = {
  data: {
...
@Component({
  ...
  name: 'CampusFailCriteria-add-edit-model',
  template: require('./index.html')
})
export class ExaminationCampusFailCriteriaMapping extends Vue {
  ...
  loadFailCriteria() {
    ...
    this.failCriteriaRepo.GetFailMaster()
      .then(r => {
  ...
  cancel() {
    this.$emit("submit");
    this.$modal.hide('CampusFailCriteria-add-edit-model');
  }
  ...
  saveModel() {
    ...
    if (!this.$v.$invalid) {
      if (this.IsNewRecord) {
        this.data.campusFailCriteriaId = helper.newGuid();
