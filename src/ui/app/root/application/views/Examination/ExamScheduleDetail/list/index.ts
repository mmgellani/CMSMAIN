...
import { DDLGroupModel, DDLModel, Examcourselist, IAttendanceAttendanceDetail, IAttendanceAttendenceMaster, IAttendanceAttendenceStatus, IAttendenceData, ICampusCityVM, ICourseSection, IExamBulkUpdateVM, IExamBulkVM, IExamCourseVM, IExamDataVM, IExaminationExamDetail, IExaminationExamMaster, IExaminationExamMasterEx, IExaminationExamType, IExaminationGradingPolicy, IFeeConcession, IFeeConcessionDetail, IFeeConcessionDetailVM, IFeeScholarshipCriteriaVM, IRegistrationSectionCourseLinkVM, IScholarshipApplyVM, IScholarshipStudentModel, ISetupAdmissionType, ISetupCampus, ISetupCampusProgramVM, ISetupClass, ISetupProgramDetails, ISetupProgramDetailsVM, ISetupSession, ISetupShift, IStudentModel, IStudentToEnrollVM, RegistrationProgramCourseLinkVM } from '../../../../models';
...
import { ExaminationExamScheduleList, IExaminationExamSchedule } from '../../../../models/Examination/ExamSchedule';
...
export class ExamScheduleDetail extends Vue {
    ...
    private examMasterEx: IExaminationExamMasterEx;
    ...
    private courseLists: Array<Examcourselist> = [];
    ...
    private repoExamSchedule: ExaminationExamScheduleService = new ExaminationExamScheduleService(this.$store)
    private examScheduleList: ExaminationExamScheduleList[] = []
    ...
    created() {
        ...
        //this.statusList= [{idd : 6 , text : 'Approved' } , {idd : 7 , text :'Marked' } , {idd : 8 , text : 'Scheduled'}]
    ...
    //this.$watch('examTypeId', this.loadExamSchedule)
        ...
        this.$watch('classId', this.loadSection)
         this.$watch('examTypeId', this.getExamTypeId)

        // this.$watch('examScheduleId', this.refreshData)

        this.$watch('programCourseLinkId', this.getExamTypeId)
    ...
    loadExamSchedule() {
        ...
        if (this.sectionCourseLinkId.length > 0) {
            ...
            console.log("Key" , key);
            this.repoExamSchedule.GetassmentexamSceduleList( key )
                .then(r => {
                    this.examScheduleList = r as ExaminationExamScheduleList[]
                })
        }
    ...
    loadSectionCourse(){
        ...
        this.courseLists = []; 
        this.examname=this.fullName;
        this.sectionCourseLinkId=this.examScheduleList.find(s => s.fullName == this.fullName).sectionCourseLinkId;

        if (this.idd == 0){
            var key = this.sectionCourseLinkId+ "?"+this.examname + '?' + 'Assessment';
            this.isAssessment = true;
        }
        else{
            var key=this.sectionCourseLinkId+ "?"+this.examname + '?' + 'Exam';
            this.isAssessment = false;
        }

        // this.programCourseLinkId = ''
                //if (this.programDetailId.length > 0 && this.classId.length > 0) {
            // var key = this.sectionCourseLinkId+ "?"+this.examname
            this.programCourseRepo.GetExamCourseList(key)
                .then(r => {
                    this.courseLists = r as Array<Examcourselist>
                });
        //}
     }
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('examScheduleDetail' in this.user.claims) == true) {
                if (this.user.claims['examScheduleDetail'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['examScheduleDetail'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['examScheduleDetail'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['examScheduleDetail'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    getExamTypeId() {
        this.examScheduleId='';
         this.examTypeId = this.examScheduleList.find(s => s.fullName == this.fullName).examTypeId;
         this.totalMarks = this.courseLists.find(s => s.programCourseLinkId == this.programCourseLinkId).totalMarks;
        //   this.examScheduleId = this.courseLists.find(e => e.programCourseLinkId === this.programCourseLinkId).examScheduleId;
    ...
    refreshData() {
        ...
        const course = this.courseLists.find(e => e.programCourseLinkId === this.programCourseLinkId);
        this.examScheduleId = course ? course.examScheduleId : '';
        this.examTypeId = this.examScheduleList.find(s => s.fullName == this.fullName).examTypeId;

        if (this.sectionCourseLinkId.length > 0 && this.programCourseLinkId.length > 0 && this.examTypeId.length > 0 && +this.examScheduleId.length > 0) {
            console.log("sectionCourseLinkId:", this.sectionCourseLinkId);
            console.log("programCourseLinkId:", this.programCourseLinkId);
            console.log("examTypeId:", this.examTypeId);
            console.log("datestring:", this.datestring);
            console.log("examScheduleId:", this.examScheduleId);

            var key = this.sectionCourseLinkId + "?" + this.programCourseLinkId + "?" + this.examTypeId + "?" +this.datestring+ "?" + this.examScheduleId;
            console.log("key:", key);


            var key = this.sectionCourseLinkId + "?" + this.programCourseLinkId + "?" + this.examTypeId + "?" +this.datestring + "?" + this.examScheduleId;
            this.examDetailRepo.GetExamScheduleUpdateData(key)
                .then(r => {
    ...
    insertModel() {
        ...
        if (this.examUpdateDataList.length == 0) {
            ...
            if (this.datas[0].isApproved) {
                this.$store.dispatch(StoreTypes.updateStatusBar, {
                    text: 'Cannot make changes Exam Already Approved',
                    title: 'Exam Already Approved',
                ...
                if (this.validMarks) {
                    ...
                    this.examMasterEx = { operation: JSON.stringify(operation), examMasterId: examMasterId, sectionCourseLinkId: this.sectionCourseLinkId, programCourseLinkId: this.programCourseLinkId, loggerId: helper.newGuid(), isAssessment: this.isAssessment, statusId: 1, dated: this.date, examTypeId: this.examTypeId, totalMarks: this.totalMarks, isApproved: false }
                    ...
                    var key = JSON.stringify(this.examMasterEx) + "?" + JSON.stringify(this.examDetailList)
...
export interface IOperationExamMasterEx {
    approvedBy: number,
    insertedBy: number,
    browserInfo: string,
    approvalTime: string,
