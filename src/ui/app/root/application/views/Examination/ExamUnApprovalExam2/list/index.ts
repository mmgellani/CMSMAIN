...
export class ExamUnApprovalExam2 extends Vue {
    ...
    private examscheduleListNew: Array<IExamScheduleNameNew> = [];
   ...
   get canSave(){
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('examUnApprovalexam2' in this.user.claims) == true) {
                if (this.user.claims['examUnApprovalexam2'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['examUnApprovalexam2'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['examUnApprovalexam2'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['examUnApprovalexam2'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        // if (this.dateMonth.toString().length > 0) {
            if (this.sectionCourseLinkId.length > 0 && this.examTypeId.length > 0) {
                ...
                if(this.idd == 0){
                    var key = this.sectionCourseLinkId + "?" + this.examTypeId + "?" + "Assessment";
                }
                else{
                    var key = this.sectionCourseLinkId + "?" + this.examTypeId + "?" + "Exam";
                }
                //var key = this.sectionCourseLinkId + "?"  + this.examTypeId
                this.examMasterRepo.GetExamUnApprovalDataExam2(key)
                    .then(response => {
    ...
    loadExamType() {
        if (this.sectionCourseLinkId.length > 0 && this.idd != null) {
            ...
            this.examMasterRepo.GetExamAssessmentScheduleNameNewList(key)
                .then(r => {
                    this.examscheduleListNew = r as Array<IExamScheduleNameNew>
                });
        }
    }
    ...
    insertModel() {
        ...
        if (ides.length > 0) {
            this.examMasterRepo.UnApprovedExam(ides)
                .then(r => {
                    this.$store.dispatch(StoreTypes.updateStatusBar, {
                        text: 'Un Approved Successfully',
                        title: '',
                        messageTypeId: PayloadMessageTypes.success
                    });
                    this.refreshData();
