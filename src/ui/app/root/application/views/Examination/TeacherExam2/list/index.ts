...
export class TeacherExam2 extends Vue {
    ...
    loadExamSchedule() {
            ...
            if (this.sectionCourseLinkId.length > 0 && this.programCourseLinkId.length > 0) {
                ...
                if (this.idd == 0){
                    var key = this.sectionCourseLinkId + '?' + this.programCourseLinkId + '?' + 'Assessment';
                    this.isAssessment = true;
                }
                else{
                    var key=this.sectionCourseLinkId + '?' + this.courseId+ '?' + 'Exam';
                    this.isAssessment = false;
                }
                console.log("Key" , key);
                this.repoExamSchedule.GetExamAssScheduleBySectionCourseLinkId( key )
                    .then(r => {
        ...
        //     this.repoExamSchedule.GetExamScheduleBySectionCourseLinkId(key)
        //         .then(r => {
        //             this.examScheduleList = r as IExaminationExamSchedule[]
        //         })
        // }
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('teacherExam2' in this.user.claims) == true) {
                if (this.user.claims['teacherExam2'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['teacherExam2'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['teacherExam2'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['teacherExam2'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    //     this.totalMarks = this.examScheduleList[0].totalMarks;
    //     this.examTypeId = this.examScheduleList[0].examTypeId;
    //     this.datas = [];
    //     this.examData = [];
    //     this.examUpdateDataList = [];
    //     if (this.examScheduleId.length > 0) {
    //         // var key = this.sectionCourseLinkId + "?" + this.programCourseLinkId + "?" + this.examTypeId + "?" + helper.formateDate(this.datestring);
    //         this.examDetailRepo.GetExamUpdateDataExam2(this.examScheduleId)
    //             .then(r => {
    ...
    refreshData() {
        ...
        this.examUpdateDataList = [];
        if (this.examScheduleId.length > 0) {
            var selectedExam = this.examScheduleList.filter(e => e.examScheduleId == this.examScheduleId)[0];

            this.totalMarks = selectedExam.totalMarks;
            this.examTypeId = selectedExam.examTypeId;
                ...
                .then(r => {
                    ...
                    if (this.examUpdateDataList.length == 0) {
                        if (this.sectionCourseLinkId.length > 0) {
                            ...
                            var key=this.sessionId+"?"+this.classId+"?"+this.sectionCourseLinkId+"?"+this.programCourseLinkId+"?"+this.examScheduleId+"?"+this.idd;
                            this.examDetailRepo.GetTeacherExamDataEX(key)
                                .then(response => {
                                    this.datas = (response as Array<IExamBulkVM>)
                                    this.totalMarks = this.datas[0].totalMarks;
                                });
                        }
                        console.log('new record')
                    }
