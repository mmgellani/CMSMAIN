...
export class AttendanceAttendanceTeacher extends Vue {
  ...
  private attenddata:SubmitAttendanceLecture={
    TimeTableId:'',
    Enrollment:[]
  }
  ...
  uploadExcel(providedData) {
    ...
    try {
     ...
     this.keys[0]=this.keys[0].replace(/\s/g, "");
      ...
      console.log(JSON.stringify(this.rollBody));
      this.rollBody.forEach(element => {
        this.rollNumbers.push({ rollNo: this.creatRollno(element.FullName) });
  ...
  saveModel() {
    ...
    var absentid2 = this.attendanceStatusList.find(s => s.fullName.toLowerCase().startsWith('a')).attendenceStatusId;
          ...
          res => {
            ...
            if (this.datas[0].val == 0) {
              ...
              this.data.forEach(e => {
                ...
                if(e.attendenceStatusId==absentid2)
                {
                this.attenddata.Enrollment.push({
                  EnrollmentId:e.rollNo

                })
              
            }
          })
              this.attenddata.TimeTableId=this.data[0].timeTableId;

              console.log(JSON.stringify(this.attenddata))
      
              
              // var key = JSON.stringify(this.attendanceMaster) + "?" + JSON.stringify(this.attendanceDetailList)
              // console.log(key)

              this.attendanceDetailRepo.AddBulkAttendenceTeacher(this.attenddata)
                .then(r => {
                  this.$store.dispatch(StoreTypes.updateStatusBar, {
...
export interface SubmitAttendanceLecture
{
