...
export class AttendanceTeacherBulk extends Vue {
        ...
        if (param == 'SessionId') {
            if (isChecked) {
                if (this.paramList.find(s => s.name == 'SessionId')) {
                    ...
                    this.paramList.push({ param: "AND cv.\"SessionId\"", value: this.sessionId, name: 'SessionId' });
                }
            } else {
                if (this.paramList.find(s => s.name == 'SessionId')) {
        ...
        if (param == 'Ribbon') {
            ...
            if (isChecked) {
                if (this.paramList.find(s => s.name == 'Ribbon')) {
                    ...
                    this.paramList.push({ param: "AND cv.\"Ribbon\"", value: this.attndanceId.toString(), name: 'Ribbon' });
                }
            } else {
                if (this.paramList.find(s => s.name == 'Ribbon')) {
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('attendanceTeacherBulk' in this.user.claims) == true) {
                if (this.user.claims['attendanceTeacherBulk'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['attendanceTeacherBulk'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['attendanceTeacherBulk'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['attendanceTeacherBulk'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    loadTimeTableData() {
        ...
        var dayname= new Date(this.datestring).getDay
        var where = "cv.\"DayName\"=''#DAYNAME#'' AND cv.\"UserId\"="+this.user.userId

       // var where = "cv.\"Dated\"=''" + moment(this.datestring).format('YYYY-MM-DD') + "''"
        //var where = "cv.\"TimeTableId\" IS NOT NULL "
        ...
        //where = where + " AND cv.\"Dated\"=" + moment(this.datestring).format('YYYY-MM-DD')
        console.log(where)
        this.timeTableDataList = [];
        this.timeTableRepo.GetTimeTableData(where+"?"+moment(this.datestring).format('YYYY-MM-DD'))
            .then(response => {
    ...
    attendanceModel(model: ITimeTableTimeTableTeacher) {
        ...
        this.$modal.show('attendance-student', { model: model, IsNewRecord: false,selectedDate:moment(this.datestring).format('YYYY-MM-DD') });
    }
