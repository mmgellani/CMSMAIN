...
import { AttendanceAttendanceDetailService, AttendanceAttendenceMasterService, AttendanceAttendenceStatusService, SetupCampusProgramLinkService } from "../../../../service";
import {
...
export class AttendanceAttendanceTeacher extends Vue {
  ...
  beforeModalOpen(event) {
    ...
    this.datestring = event.params.dated;


    var key1 = moment(this.datestring).format("YYYY-MM-DD") + "?" + this.datas.timeTableId;
    ...
    if (this.datas.campusId.length > 0 && this.datas.sessionId.length > 0 && this.datas.programDetailId.length > 0) {       // this.campusCityList.find(s=>s.)
      ...
      this.Programrepository.GetAllActive(key).then(res => {
        ...
        if (this.ProgramList[0].campusProgramId.length > 0 && this.datas.dayName.length > 0 && this.datas.timeTableId.length > 0) {
          var key = moment(this.datestring).format('YYYY-MM-DD') + '?' + this.datas.timeTableId
  ...
  saveModel() {
    if (this.detailData[0] != null) {
      ...
      if (Array.from(new Set(this.Ddata.map((item: any) => item.timeTableId))).length == 1) {
        ...
        var key = moment(this.datestring).format('YYYY-MM-DD') + '?' + this.datas.campusId + '?' + this.datas.sessionId + '?' + this.datas.sectionId + '?' + this.datas.classId + '?' + this.datas.programDetailId + '?' + this.datas.courseId + '?' + this.datas.slotTimingId;
        // alert(key);
        this.CheckRepository.CheckClash(key).then(
          res => {
            ...
            if (this.datas[0].val == 0) {
              ...
              this.attendanceMaster = { attendenceMasterId: atndanceMasterId, isApproved: false, loggerId: helper.newGuid(), statusId: 1, dated: new Date(moment(this.datestring).format('YYYY-MM-DD')), timeTableId: this.data[0].timeTableId, operation: JSON.stringify(this.opertation) }
              this.data.forEach(e => {
  ...
  DisableonLeave(option: any) {
    var z = this.attendanceStatusList.find(e => e.attendenceStatusId == option).fullName;
    if (z == 'Leave')
      return true;
    return false;
