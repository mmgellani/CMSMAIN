...
import { IAssessmentSchemeDefinitionAll,IAssessmentSchemeMasterList,IAssessmentSchemeDetailListGet,IAssessmentSchemeDefinitionAllEx } from '../../../../models/Setup/AssessmentSchemeDefinition';
...
export class AssessmentSchemeDefinition extends Vue {
...
private AssesmentSchmeDetailRepo   :AssessmentSchemeDetailService;
    ...
    private assesmentScemeDeatilist:Array<IAssessmentSchemeDetailListGet>=[];
    ...
    private columns = [
        { key: 'fullName', caption: 'Assessment SchemeName' },
        // { key: 'totalWeightage', caption: 'Total Weightagee' },
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('assessmentSchemeDefinition' in this.user.claims) == true) {
                if (this.user.claims['assessmentSchemeDefinition'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['assessmentSchemeDefinition'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['assessmentSchemeDefinition'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['assessmentSchemeDefinition'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
...
debugger
    ...
    editModel(model: IAssessmentSchemeDefinitionAllEx) {
        
        this.$modal.show('ExamSchedule-add-edit-model', { model: model, IsNewRecord: false });
       // var masterid=model.assessmentSchemeMasterId;
...
this.AssesmentSchmeDetailRepo.GetAssesmentDetailData(key).then(r => {
  this.assesmentScemeDeatilist = r as Array<IAssessmentSchemeDetailListGet>;
console.log(id);
});
    }
    loadAssesmentMasterList(id:any) {
        ...
        this.AssesmentSchmeDetailRepo.GetAssesmentMasterData(key).then(r => {
          this.assesmentScemeMasterlist = r as Array<IAssessmentSchemeMasterList>;
        });
      }

      deleteModel(model: IAssessmentSchemeDefinitionAllEx) {
