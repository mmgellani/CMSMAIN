...
export class AssessmentSchemeDetail extends Vue {
    ...
    created() {
        ...
        this.$watch('examScheduleId', this.getExamTypeId)
        this.$watch('examScheduleId', this.refreshData)


    }
    loadExamSchedule() {
        if (this.sectionCourseLinkId.length > 0 && this.courseId.length > 0) {
            this.repoExamSchedule.GetFindBy('s=>s.StatusId==1 && s.SectionCourseLinkId.ToString()=="' + this.sectionCourseLinkId + '" && s.CourseId.ToString()=="' + this.courseId + '"')
                .then(r => {
                    this.examScheduleList = r as IExaminationExamSchedule[]
                })
        }
    ...
    getExamTypeId() {
        this.examTypeId = this.examScheduleList.find(s => s.examScheduleId == this.examScheduleId).examTypeId;
        this.totalMarks = this.examScheduleList.find(s => s.examScheduleId == this.examScheduleId).totalMarks;
        console.log(this.examScheduleList.find(s => s.examScheduleId == this.examScheduleId).totalMarks)
    }
    refreshData2() {
