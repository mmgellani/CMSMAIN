...
import { RolePrevilageEdit } from '../single-edit';
import { Signup } from '../../../../../../admin/users/signup/signup';
import { SignupU } from '../signup/signup';
...
export class RolePrevilagesList extends Vue {
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('RolePrevilages' in this.user.claims) == true) {
                if (this.user.claims['RolePrevilages'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['RolePrevilages'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['RolePrevilages'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['RolePrevilages'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    insertModel() {
        this.$modal.show('add-edit-model-user');
