...
export class TeacherHod extends Vue {
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("Teacherhod" in this.user.claims == true) {
        if (this.user.claims["Teacherhod"].indexOf("R") >= 0) {
          this.canRead = true;
        }
        if (this.user.claims["Teacherhod"].indexOf("C") >= 0) {
          this.canAdd = true;
        }
        if (this.user.claims["Teacherhod"].indexOf("U") >= 0) {
          this.canEdit = true;
        }
        if (this.user.claims["Teacherhod"].indexOf("D") >= 0) {
          this.canDelete = true;
        }
      } else {
  ...
  insertModel() {
    ...
    if (this.sessionid.length > 0 && this.campusid.length > 0 && this.campusProgramId.length > 0 && this.classid.length > 0) {
      this.programdetailid = this.campusProgramLinkList.find(e => e.sessionId == this.sessionid && e.campusId == this.campusid && e.campusProgramId == this.campusProgramId).programDetailId;
      this.$modal.show('staff-rec', { PROGRAMDETAIL: this.programdetailid, CAMPUSPROGRAMID: this.campusProgramId, COURSEID: this.courseid, CLASSID: this.classid });
    }
    else {
  ...
  deleteModel(option) {
    ...
    if (response) {
      this.staffService.DeleteHODData(option.ide).then(r => {
        this.$store.dispatch(StoreTypes.updateStatusBar, {
          text: r,
          title: 'Success',
          messageTypeId: PayloadMessageTypes.error
  ...
  GetHOdList() {
    ...
    var key = this.campusProgramId + '?' + this.programdetailid + '?' + this.courseid + '?' + this.classid;
    this.staffService.GetHODData(key).then(r => {
