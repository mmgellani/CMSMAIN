...
export class ExaminationReports extends Vue {
  ...
  getExamIndividSummaryDetail() {
    ...
    this.displayList = []
    this.repository.GetExamIndividSummaryE12(key).then(response => {
  ...
  getExamMonthlyExx(key) {
    this.reportData = [];
    this.repository.GetExamMonthlyExx(key).then(response => {
      if (response == null || response == [] || response == 0) {
        this.$store.dispatch(StoreTypes.updateStatusBar, {
          text: "No Record Found",
          title: "Warning",
        ...
        this.$store.dispatch(RootStoreTypes.reportOperation, {
          data: this.reportData as any,
          path: '/assets/Reports/Resource/Examination/exam-section-summary.xml',
          show: true
        });
      }
    });
