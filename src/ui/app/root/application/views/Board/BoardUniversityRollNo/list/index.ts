...
import { IBoardStudentBoardLink, StudentBoardLinkData, StudentBoardRegistration, StudentBoardExamEntry, BoardUniversityRollData, StudentBoardUniversityRollNoList, BoardUniRolNoslip } from '../../../../models/Board/StudentBoardLink';
...
export class BoardUniversityRollNo extends Vue {
    ...
    private dataEx: BoardUniRolNoslip = {
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('boardUniversityRollNo' in this.user.claims) == true) {
                if (this.user.claims['boardUniversityRollNo'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['boardUniversityRollNo'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['boardUniversityRollNo'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['boardUniversityRollNo'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if(this.sessionId.length>0 && this.campusId.length>0 && this.programId.length>0 && this.classId.length>0 && this.programDetailIdEx.length>0  && this.examtypeid.length>0 && this.year.length>0)
        {
        var key=this.sessionId+'?'+this.campusId+'?'+this.programId+'?'+this.classId+'?'+this.programDetailIdEx+'?'+this.examtypeid+'?'+this.year
        this.repository.GetBoardUniversityRollList(key).then(r=>{
    ...
    insertModel() {
        if (this.classId.length > 0) {
            ...
            this.$modal.show('StudentRegistration-add-edit-model', {
                ...
                SessionId: this.sessionId, CampusId: this.campusId, ProgramDetailId: this.programId, ClassId: this.classId, PROGRAMDETAILID: this.programDetailId, IsNewRecord: true, PROGRAMID: this.programId,SessionName:this.sessionName,EXMYEAR:this.year,EXAMTYPE:this.examtypeid
            });
        }
        else {
            this.$store.dispatch(StoreTypes.updateStatusBar, {
    ...
    deleteconfirmation() {
        ...
        this.repository.UpdateboardRollNoslip(this.dataEx).then(r => {
