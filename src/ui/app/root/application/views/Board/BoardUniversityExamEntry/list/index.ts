...
import { IBoardStudentBoardLink, StudentBoardLinkData, StudentBoardRegistration, StudentBoardExamEntry } from '../../../../models/Board/StudentBoardLink';
...
export class BoardUniversityExamEntry extends Vue {
    ...
    private datalist:Array<StudentBoardExamEntry>=[];
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('boardUniversityExamEntry' in this.user.claims) == true) {
                if (this.user.claims['boardUniversityExamEntry'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['boardUniversityExamEntry'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['boardUniversityExamEntry'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['boardUniversityExamEntry'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if(this.registrationCodeId.length==0)
        {
            this.registrationCodeId='00000000-0000-0000-0000-000000000000';
        }
        if(this.sessionId.length>0 && this.campusId.length>0 && this.programId.length>0 && this.classId.length>0 && this.programDetailIdEx.length>0 && this.registrationCodeId.length>0 && this.examtypeid.length>0 && this.year.length>0)
        {
        ...
        this.repository.GetBoardUniversityExamEx(key).then(r=>{
         this.datalist= r as Array<StudentBoardExamEntry>
    ...
    insertModel() {
        if (this.classId.length > 0) {
            ...
            this.$modal.show('StudentRegistration-add-edit-model', {
                ...
                SessionId: this.sessionId, CampusId: this.campusId, ProgramDetailId: this.programId, ClassId: this.classId, PROGRAMDETAILID: this.programDetailId, IsNewRecord: true, PROGRAMID: this.programId, RegistrationCodeId : this.registrationCodeId,SessionName:this.sessionName,EXMYEAR:this.year,EXAMTYPE:this.examtypeid
            });
        }
        else {
            this.$store.dispatch(StoreTypes.updateStatusBar, {
    ...
    deleteconfirmation() {
        ...
        this.repository.DeleteStudentBoardExamEntry(key).then(r => {
