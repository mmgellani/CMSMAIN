...
import { IUser, PayloadMessageTypes } from '../../../../../../model';
import { IRootStoreState } from '../../../../../store';
import {   BoardExamTypeAddEdit } from '../add-edit';
import {  BoardExamTypeDelete } from '../delete';
...
@Component({
    ...
    components: {
        'add-edit-model': BoardExamTypeAddEdit,
        'delete-model': BoardExamTypeDelete
    }
})

export class SessionBoardExamTypeList extends Vue {
    @State((state: IRootStoreState) => state.common.user) user: IUser;

    private repository: SessionBoardExamTypeService;
    private data: Array<IBoardExamType> = [];
    ...
    private columns = [
        { key: 'fullName', caption: 'Exam Type Name' },
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('SessionBoardExamTypeList' in this.user.claims) == true) {
                if (this.user.claims['SessionBoardExamTypeList'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['SessionBoardExamTypeList'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['SessionBoardExamTypeList'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['SessionBoardExamTypeList'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        this.repository.GetFindBy("e=>e.StatusId==1")
            .then(response => this.data = (response as Array<IBoardExamType>));
    }

    insertModel() {
       
            this.$modal.show('add-edit-model', { model: { examTypeId: '', fullName: '', description: '' , statusId: 1}, IsNewRecord: true });
