...
export class UnApprove extends Vue {
    ...
    beforeModalOpen(event) {
       ...
       // this.data.bulkNotificationId= event.params.model.bulkNotificationId;
    ...
    updMicrosoftPass() {
        var key = this.data.messageText + '?' + moment(this.data.quedDate).format("YYYY/MM/DD")  + '?' + this.data.userId;

        this.service.UnApprove(key).then(r => {
            this.$store.dispatch(StoreTypes.updateStatusBar, {
                text: 'Notification reverted successfully',
                title: '',
