...
type ValidateSetupPassStatus = {
  data: ISetupPassStatus;
  validationGroup: string[];
};
let customValidation: ValidationRuleset<ValidateSetupPassStatus> = {
  data: {
    fullName: { required }
  }
};
...
export class SetupPassStatusAddEdit extends Vue {
  private repository: SetupPassStatusService;
  isActive: boolean = true;
  private data: ISetupPassStatus = {
    passStatusId: "",
    fullName: "",
    statusId: 0,
    loggerId: ""
  ...
  saveModel() {
    ...
    if (!this.$v.$invalid) {
      if (this.IsNewRecord) {
        ...
        this.data.passStatusId = helper.newGuid();
