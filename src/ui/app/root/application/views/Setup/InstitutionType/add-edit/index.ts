...
@Component({
  ...
  name: "InstitutionType-add-edit-model",
  template: require("./index.html")
})
export class SetupInstitutionTypeAddEdit extends Vue {
  ...
  cancel() {
    this.$emit("submit");
    this.$modal.hide("InstitutionType-add-edit-model");
  }
  ...
  saveModel() {
    ...
    if (!this.$v.$invalid) {
      if (this.IsNewRecord) {
        this.data.institutionTypeID = helper.newGuid();
  ...
  get allowSubmit() {
    return this.data.description.length > 0 && this.data.name.length > 0;
  }
  $v: any;
}
