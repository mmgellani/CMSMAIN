...
type ValidateSetupGroup = { data: ISetupGroup; validationGroup: string[] };
let customValidation: ValidationRuleset<ValidateSetupGroup> = {
  data: {
    fullName: {
      required,
...
export class SetupGroupAddEdit extends Vue {
  ...
  saveModel() {
    ...
    if (!this.$v.$invalid) {
      if (this.IsNewRecord) {
        this.data.groupId = helper.newGuid();
