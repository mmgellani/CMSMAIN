...
import { IFeeFeeHead, IFeeStudentFeeStructureVM, IFeeStudentChallanVM, ISetupCollector, IFeeStudentChallan, IFilterString } from '../../../../models';
...
import { StudentDisableChallanSearch } from '../DisableChallan';
...
@Component({
    ...
    components: {
        WidgetBox,
        'disable-challan': StudentDisableChallanSearch
    }
})

export class StudentFeeChallanSearch extends Vue {
    ...
    SearchFeeConfirmation() {
        ...
        if (!this.$v.$invalid) {
            ...
            this.FeeChallanrepository.GetSearchBychallan(this.data.filterString).then(res => {
        ...
        this.FeeChallanrepository.GetAllChallandata(this.data.filterString).then(
            res => {
                ...
                if (this.StudentFeedata[0].installmentNo == 1) {
                    ...
                    if (this.StudentFeedata.length > 0) {
                        var filterkey = this.StudentFeedata[0].admissionFormId + '?' + (this.StudentFeedata[0].installmentNo - 1);
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('feeChallanSearch' in this.user.claims) == true) {
                if (this.user.claims['feeChallanSearch'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['feeChallanSearch'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['feeChallanSearch'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['feeChallanSearch'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
