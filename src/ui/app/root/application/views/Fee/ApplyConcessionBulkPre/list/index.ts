...
export class FeeApplyConcessionBulkPre extends Vue {
  ...
  maxsubInstallment = [{ item: 10 }, { item: 1500 }, { item: 2500 }, { item: 5000 },
    { item: 7500 }, { item: 10000 },{ item: 10500 }, { item: 15000 }, { item: 20000 }, { item: 25000 }]
  created() {
    this.loadGender();
  ...
  loadProgramsOfCampus() {
    ...
    this.campusProgramLinkRepo.GetAllVM(key).then(r => {
      ...
      this.campusProgramLinkList = this.campusProgramLinkList.filter(e=> e.description.includes('Pre '))
    });
  }
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("applyBulkChallanPre" in this.user.claims == true) {
        if (this.user.claims["applyBulkChallanPre"].indexOf("R") >= 0) {
          this.canRead = true;
        }
        if (this.user.claims["applyBulkChallanPre"].indexOf("C") >= 0) {
          this.canAdd = true;
        }
        if (this.user.claims["applyBulkChallanPre"].indexOf("U") >= 0) {
          this.canEdit = true;
        }
        if (this.user.claims["applyBulkChallanPre"].indexOf("D") >= 0) {
          this.canDelete = true;
        }
      } else {
  ...
  refreshData() {
    ...
    if (
      ...
      // "?" +
      // this.admissionTypeId;
      this.repository.GetStudentsPre(key).then(response => {
        this.selected = [];
  ...
  saveValidation(){
    if(this.selected.length==0 || this.installmentAmount == 0)
    {
      return true;
    }
    else{
  ...
  insertModel() {
    ...
    if (this.selected.length > 0) {
      var key = this.installmentAmount + "?" + JSON.stringify(list) + "?" + this.installmentNo;
      // alert(JSON.stringify(key));
      this.concessionDetailRepo.ApplySubInstallmentBulkPre(key).then(r => {
        // console.log('done')
