...
import { CustomChallanAddEdit } from '../add-edit';
...
@Component({
    ...
    components: {
        'add-edit-model': CustomChallanAddEdit
    }
})

export class CustomChallanList extends Vue {
   ...
   //  alert(k);
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('customChallan' in this.user.claims) == true) {
                if (this.user.claims['customChallan'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['customChallan'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['customChallan'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['customChallan'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
