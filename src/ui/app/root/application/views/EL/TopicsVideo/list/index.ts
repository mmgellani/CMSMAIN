...
export class TopicsVideoList extends Vue {
  @State((state: IRootStoreState) => state.common.user) user: IUser;

  private repository: TopicsVideoService = new TopicsVideoService(this.$store);
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("TopicsVideo" in this.user.claims == true) {
        if (this.user.claims["TopicsVideo"].indexOf("R") >= 0) {
          this.canRead = true;
        }
        if (this.user.claims["TopicsVideo"].indexOf("C") >= 0) {
          this.canAdd = true;
        }
        if (this.user.claims["TopicsVideo"].indexOf("U") >= 0) {
          this.canEdit = true;
        }
        if (this.user.claims["TopicsVideo"].indexOf("D") >= 0) {
          this.canDelete = true;
        }
      } else {
