...
export class RegistrationClassPromotionList extends Vue {
    ...
    EnableDisableCampus() {
        var frid = this.campusCityList.find(e => e.campusId == this.campusId).franchiseId;
        if (frid === "cb195478-04c1-4f2d-b2ba-f7088c57ea88") {
            this.disableCampus = false;
            this.disableProgram = false;
        }
        else {
            this.disableCampus = true;
            this.disableProgram = true;
        }
    }
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('classPromotion' in this.user.claims) == true) {
                if (this.user.claims['classPromotion'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['classPromotion'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['classPromotion'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['classPromotion'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if (this.campusProgramId.length > 0 && this.classid.length > 0 && this.sectionCourseLinkId.length > 0) {
            ...
            this.enrollmentRepo.GetStudentPromotionList(key)
                .then(response => {
    ...
    insertModel() {
        ...
        if (this.sessionId == this.tosessionId && this.sessionId.length > 0 && this.tosessionId.length > 0) {
            ...
            if (z.length > 0) {
                var key = JSON.stringify(z) + '?' + this.transferclassid + '?' + this.tocampusProgramId;
                this.enrollmentRepo.InsertClassPromotionEx(key).then(r => {
