...
export class RegistrationSectionChangeList extends Vue {
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('sectionChange' in this.user.claims) == true) {
                if (this.user.claims['sectionChange'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['sectionChange'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['sectionChange'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['sectionChange'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    insertModel(model: IStudentEnrolledVM) {

        // var response = confirm('Are you sure to Change Section of this student?');
        // if (response) {

        var key = model.enrollmentId + '?' + model.sectionCourseLinkId;

        this.enrollmentRepo.UpdateScetion(key)
            .then(() => {
                this.$store.dispatch(StoreTypes.updateStatusBar, {
