...
export class RegistrationEnrollmentBulkWithoutPaidList extends Vue {
    ...
    private repoConcessionDetail: FeeConcessionDetailService = new FeeConcessionDetailService(this.$store)
    private concessionList: Array<IConcessionVM> = []
    private columns = [
        { key: 'RefrenceNo', caption: 'Reference No.' },
    ...
    created() {
        ...
        this.$watch('campusProgramId', this.loadConcessions)
        this.$watch('admissionTypeId', this.loadConcessions)
    }
    ...
    loadTransfer() {
        this.repository.GetHadafTransfer()
            .then(
    ...
    loadConcessions() {
        if (this.campusProgramId.length > 0 && this.admissionTypeId.length > 0) {
            this.repoConcessionDetail.GetConcession(this.campusProgramId + "?" + this.admissionTypeId)
                .then(r => {
                    this.concessionList = r as Array<IConcessionVM>
                })
        }
    }
    loadSections() {
        if (this.classid.length > 0) {
            var key2 = this.campusProgramId + '?' + this.classid
    ...
    refreshData() {
        console.log('CampusProgramid' + this.campusProgramId)
        var foundData = this.trans.filter(e => e.toid == this.campusProgramId);

        if (foundData) {
            var getStr = { "providedString": foundData[0].fromid };

            console.log(JSON.stringify(getStr));
            this.datas = [];

            this.repository.GetHadafData(getStr)
                .then(response => {
                    ...
                    this.datas.forEach(element => {
                        ...
                        element.GroupId = this.sectionId;
                        element.LoggerId = this.classid;
    ...
    insertModel() {
        ...
        this.repository.InsertHadafList(getStr)
