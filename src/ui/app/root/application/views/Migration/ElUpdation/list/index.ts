...
export class ElUpdationList extends Vue {
...
this.loadBatch();
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('elUpdation' in this.user.claims) == true) {
                if (this.user.claims['elUpdation'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['elUpdation'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['elUpdation'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['elUpdation'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if (this.campusProgramId.length > 0) {
            ...
            this.admissionRepo.GetElStudentEx(key)
                .then(response => {
    ...
    UpdateElRecord(option:any)
    {
        ...
        this.$modal.show('el-model',{ROLNO:option,campusId:this.campusId,sessionId:this.sessionId});
