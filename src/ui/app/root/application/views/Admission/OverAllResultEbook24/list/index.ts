...
export class OverAllResultEbook24 extends Vue {
  ...
  isResetDisabled() {
    ...
    return !this.typeId && !this.cityId && !this.subCityId;
  }
  loadPosessionDataSurvey(id) {
    if (this.typeId == "Owned") {
      ...
      this.repository.SurveyStatisticsEx1LatestEBook(key).then((r) => {
        if (r) {
      ...
      this.repository.SurveyStatisticsEx1LatestJun24(key).then((r) => {
        if (r) {
  ...
  loadCityDataSurvey(id) {
    ...
    if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
      ...
      var key = this.subCityId + "?" + 'building' + "?" + this.user.userId + "?" + 'e03292cf-244e-4435-b530-4ff0653bae4c' + "?" + 'bb638a38-670c-4471-bf45-4ce602404cd8' + "?" + this.buildingname;
      this.repository.SurveyStatisticsCityWiseJun24WithBuilding(key).then((r) => {
      ...
      var key = id + "?" + 'city' + "?" + this.user.userId + "?" + 'f36f5089-c160-4e97-902d-48b230c85e6a' + "?" + '00000000-0000-0000-0000-000000000000';
      this.repository.GetSurveyCityEbook24(key).then((r) => {
  ...
  loadSubCityData(id) {
    ...
    if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
      ...
      debugger;
      this.showloader = true;
      var key = id + "?" + 'subcity' + "?" + this.user.userId + "?" + 'f36f5089-c160-4e97-902d-48b230c85e6a' + "?" + '00000000-0000-0000-0000-000000000000';
      this.repository.GetSurveyCityEbook24(key).then((r) => {
        debugger;
  ...
  loadbuildingdata() {
    if (this.subCityId.length > 0) {
      ...
      this.repository.BuildingSectionDataWithSessionAndClass(this.subCityId).then((r) => {
  ...
  loadTeachers(course) {
    ...
    if (this.cityId.length == 0 && this.subCityId.length == 0 ) {
      ...
      this.repository.TeacherRatingswithidJun2024(key).then((r) => {
...
this.repository.Jun24TeacherRatingswithidSpecificCityWithBuilding(key.trim()).then((r) => {
  this.teacherRatingList = r as Array<ITeacherRatingOverAllListwithid>;
  ...
  this.teachCheck = true;
      ...
      this.repository.Jun2024TeacherRatingswithidSpecificCity(key.trim()).then((r) => {
        this.teacherRatingList = r as Array<ITeacherRatingOverAllListwithid>;
  ...
  loadSectionData(id) {
    ...
    this.repository.GetTeacherGraphSectionWisewithtotalJun24Xx(key).then((r) => {
    ...
    this.repository.GetTeacherGraphSectionWisewithtotalJun24EXx(key).then((r) => {
      this.SectionData = r as Array<ITeacherRatingGraphEXSectionwithtotal>;
      this.data1 = true;
  ...
  isPercentage(q: string) {
      ...
      console.log('dufhdsfds')
    if (q==="How likely are you to recommend these books to your peers?") {
      return true;
    } else {
      return false;
    }
  ...
  refreshData() {
    ...
    this.repository.GetOverAllResultEbook24(key).then((r) => {
  ...
  loadData() {
    ...
    ${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}?${this.buildingname}`;
          ...
          if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
            ...
            // this.repository.GetOverAllResult1Jun24WithBuilding(key.trim()).then((r) => {
            //   this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
            //   this.loadQuestions();
            //   this.loadsummery();
            // });
            ...
            // this.showloader = true;
            // this.repository.GetOverAllResult1withposissionJun2024withbuilding(key).then((r) => {
            //   this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
          ...
          this.showloader = true;
          this.repository.GetOverAllResult1withposissionEbook24(key).then(r => {
            if (r) {
...
${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}?${this.buildingname}`;
    ...
    else
    {
      ...
      ${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}`;
            this.showloader = true;
            this.repository.GetOverAllResult1Ebook24(key.trim()).then((r) => {
              if (r) {
                this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
                this.loadQuestions();
                this.loadsummery();
    ...
    this.repo.ParamsDashboard(`${module}:${param}`).then((response) => {
      switch (module) {
        case "type":
          ...
          this.campusTypeList = this.campusTypeList.filter(item => item.fullName !== 'Franchise CO' && item.fullName !== 'Franchise');
          break;
