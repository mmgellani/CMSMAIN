...
export class AdmissionAdmissionFormAddEdit extends Vue {
    ...
    if (this.datas.filterString.length >= 14 && this.datas.filterString.length <= 17) {
      this.repository.GetStudentPromoted(this.datas.filterString).then(response => {
        ...
        if (this.dataEx.length > 0) {
          //Same Session Check
          if(this.sessionId!=this.dataEx[0].sessionId){
            this.$store.dispatch(StoreTypes.updateStatusBar, {
              text: "Student Cannot be Promoted in different Session!",
              title: "error",
  ...
  loadProgramsOfCampus() {
    ...
    this.campusProgramLinkRepo.GetAllVM(key).then(r => {
      ...
      //remove pre programs
      this.campusProgramLinkList=this.campusProgramLinkList.filter(s=>!s.description.toLowerCase().startsWith('pre'))
    });
  }
  loadAdmissionData() {
  ...
  beforeModalOpen(event) {
    ...
    if (this.IsNewRecord) {
      ...
      //   this.admissionFormModel = res as Array<IAdmissionAdmissionFormVMEx>;
      // });
  ...
  getStudentReportData(challlanNo: any) {
    ...
    if (resut == 'Z') {
      ...
      var key = this.sessionId + "?" + this.campusId + "?" + this.admissionFormId;
