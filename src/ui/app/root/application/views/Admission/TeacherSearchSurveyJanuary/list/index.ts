...
export class TeacherSearchSurveyJanuary23 extends Vue {
  ...
  loadCourseData(id) {
    ...
    this.repository.GetTeacherGraphSubjectJanuary23(key).then((r) => {
  ...
  loadSectionData(id) {
    ...
    this.SectionData=[];
    var key = id + "?" + this.user.userId;
    this.repository.GetTeacherGraphSectionWisewithtotalJanuary23(key).then((r) => {
 ...
 selectedItem: any = {};
  ...
  loadTeacherData(teacherId, i) {
    ...
    this.currIndex = i;
    this.dataSurvey = [];
    this.repository.GetTeacherSurveyEXJanuary23(teacherId + "?" + this.user.userId)
      .then(response => {
  ...
  loadData(){
    ...
    if (this.cityId.length > 0 && this.subCityId.length > 0) {
      ...
      this.repository.GetTeacherSearchwithcityApril(key+ "?" + this.user.userId)
        .then(response => {
          this.data = response as Array<ITeacherSearch>
        });
    }
