...
export class TeacherSearchSurveyJune24 extends Vue {
  ...
  loadCourseData(id) {
    ...
    this.repository.GetTeacherGraphSubjectJun24(key).then((r) => {
  ...
  loadSectionData(id) {
    ...
    this.SectionData=[];
    var key = id + "?" + this.user.userId;
    this.repository.GetTeacherGraphSectionWisewithtotalJun24(key).then((r) => {
 ...
 selectedItem: any = {};
  ...
  loadTeacherData(teacherId, i) {
    ...
    this.currIndex = i;
    this.dataSurvey = [];
    this.repository.GetTeacherSurveyEXJun24(teacherId + "?" + this.user.userId)
      .then(response => {
  ...
  loadData(){
    ...
    if (this.cityId.length > 0 && this.subCityId.length > 0) {
      ...
      this.repository.GetTeacherSearchwithcityJune24(key+ "?" + this.user.userId)
        .then(response => {
          this.data = response as Array<ITeacherSearch>
        });
    }
