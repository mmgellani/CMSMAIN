...
export class OverAllResultJanuary24 extends Vue {
  ...
  loadTotalSurvey() {
    ...
    this.survey = [];
    this.repository
      .TotalSurveyJanuary2024('97db1498-2727-4716-8f7c-b5775c841b24?0f54faf5-0b88-4110-a0ec-1c65e92d0c2e?765a9861-3383-45a0-aec7-c1367a4fd1cd?399135a1-4da8-4bc2-b284-8eafb3a0d941')
      .then((response) => (this.survey = response as Array<ITotalSurveyJanuary2024>));
  ...
  loadPosessionDataSurvey(id) {
    if (this.typeId == "Owned") {
      ...
      this.repository.SurveyStatisticsEx1LatestJanuary24(key).then((r) => {
        if (r) {
  ...
  loadCityDataSurvey(id) {
    ...
    if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
      ...
      var key = this.subCityId + "?" + 'building' + "?" + this.user.userId + "?" + 'b3228914-252e-4f90-b643-0047470059a6' + "?" + '5783ad6d-a901-4cfb-b160-1a8307752302' + "?" + this.buildingname;
      this.repository.GetSurveyCityJanuary24WithBuilding(key).then((r) => {
      ...
      var key = id + "?" + 'city' + "?" + this.user.userId + "?" + 'b3228914-252e-4f90-b643-0047470059a6' + "?" + '5783ad6d-a901-4cfb-b160-1a8307752302';
      this.repository.GetSurveyCityJanuary24(key).then((r) => {
  ...
  loadSubCityData(id) {
    ...
    if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
      ...
      this.showloader = true;
      var key = id + "?" + 'subcity' + "?" + this.user.userId + "?" + 'b3228914-252e-4f90-b643-0047470059a6' + "?" + '5783ad6d-a901-4cfb-b160-1a8307752302';
      this.repository.GetSurveyCityJanuary24(key).then((r) => {
  ...
  loadbuildingdata() {
    if (this.subCityId.length > 0) {
      ...
      this.repository.BuildingSectionData24(this.subCityId).then((r) => {
  ...
  isTeacherTable(serial: number) {
    return serial == 3 && this.teachCheck === true;
  }
  ...
  loadTeachers(course) {
    ...
    if (this.cityId.length == 0 && this.subCityId.length == 0) {
      ...
      this.repository.TeacherRatingswithidJanuary2024(key).then((r) => {
  ...
  ${this.classId.length > 0 ? this.classId : this.empty}?${this.whichModule}?${course}?${this.user.userId}?${this.buildingname}`;
        ...
        this.repository.Jan24TeacherRatingswithidSpecificCityWithBuilding(key.trim()).then((r) => {
          this.teacherRatingList = r as Array<ITeacherRatingOverAllListwithid>;
        ...
        this.repository.Januray24TeacherRatingswithidSpecificCity(key.trim()).then((r) => {
          this.teacherRatingList = r as Array<ITeacherRatingOverAllListwithid>;
  ...
  loadSectionData(id) {
    ...
    if (this.cityId.length == 0 && this.subCityId.length == 0) {
      ...
      this.repository.GetTeacherGraphSectionWisewithtotalJanuary24Ex(key).then((r) => {
    ...
    ${this.subCityId.length > 0 ? this.subCityId : this.empty}?${id}?${this.user.userId}?${this.selectedCourse}`;
      this.repository.GetTeacherGraphSectionWisewithtotalJanuary24EXx(key).then((r) => {
        this.SectionData = r as Array<ITeacherRatingGraphEXSectionwithtotal>;
        this.data1 = true;
  ...
  refreshData() {
    ...
    var key = this.user.userId + "?" +
      'b3228914-252e-4f90-b643-0047470059a6' + "?" + '5783ad6d-a901-4cfb-b160-1a8307752302';
    this.repository.GetOverAllResultJanuary24(key).then((r) => {
  ...
  loadData() {
    ...
    ${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}?${this.buildingname}`;
          ...
          if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
            ...
            this.repository.GetOverAllResult1January24WithBuilding(key.trim()).then((r) => {
              this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
              this.loadQuestions();
              this.loadsummery();
            });
          ...
          this.showloader = true;
          this.repository.GetOverAllResult1withposissionJanuary24(key).then(r => {
            if (r) {
...
${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}?${this.buildingname}`;
      ...
      ${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}`;
        this.showloader = true;
        this.repository.GetOverAllResult1January24(key.trim()).then((r) => {
          if (r) {
            this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
            this.loadQuestions();
            this.loadsummery();
