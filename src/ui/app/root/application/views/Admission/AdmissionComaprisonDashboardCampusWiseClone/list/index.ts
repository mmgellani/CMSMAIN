...
import { StudentListCampusWise} from "../add-edit";
...
@Component({
  ...
  components: {
    "add-edit-model": StudentListCampusWise,
...
export class AdmissionComaprisonDashboardCampusWiseClone extends Vue {
  ...
  private fromDateEx = new Date("2021-08-14");
 ...
 // public newloader: boolean = true;
  ...
  getData(provider: string, param: string = "") {
    ...
    if (this.typeId == "Owned") {
      ...
      this.repository.GetAdmissionSessionWiseDataCampusTestingClone(key).then((r) => {
...
newgetdata(provider: string, param: string = ""){
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("admissionComaprisonDashboardCampusWise" in this.user.claims == true) {
        if (
          this.user.claims["admissionComaprisonDashboardCampusWise"].indexOf("R") >= 0
        ) {
          this.canRead = true;
        }
        if (
          this.user.claims["admissionComaprisonDashboardCampusWise"].indexOf("C") >= 0
        ) {
          this.canAdd = true;
        }
        if (
          this.user.claims["admissionComaprisonDashboardCampusWise"].indexOf("U") >= 0
        ) {
          this.canEdit = true;
        }
        if (
          this.user.claims["admissionComaprisonDashboardCampusWise"].indexOf("D") >= 0
        ) {
          this.canDelete = true;
        }
      } else {
