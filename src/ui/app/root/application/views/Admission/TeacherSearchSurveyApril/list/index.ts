...
export class TeacherSearchSurveyApril2022 extends Vue {
  ...
  loadCourseData(id) {
    ...
    this.repository.GetTeacherGraphSubjectApril(key).then((r) => {
 ...
 selectedItem: any = {};
  ...
  loadTeacherData(teacherId, i) {
    ...
    this.currIndex = i;
    this.dataSurvey = [];
    this.repository.GetTeacherSurveyExApril(teacherId + "?" + this.user.userId)
      .then(response => {
