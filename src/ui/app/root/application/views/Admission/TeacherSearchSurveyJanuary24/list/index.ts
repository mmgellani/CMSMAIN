...
export class TeacherSearchSurveyJanuary24 extends Vue {
  ...
  loadTeacherSearch() {
    ...
    if (this.filterString.length > 0) {
      ...
      this.data = [];
      this.repository.GetTeacherSearchJanuary24(this.filterString + "?" + this.user.userId)
        .then(response => {
  ...
  loadCourseData(id) {
    ...
    this.repository.GetTeacherGraphSubjectJanuary24(key).then((r) => {
  ...
  loadSectionData(id) {
    ...
    this.SectionData=[];
    var key = id + "?" + this.user.userId;
    this.repository.GetTeacherGraphSectionWisewithtotalJanuary24(key).then((r) => {
 ...
 selectedItem: any = {};
  ...
  loadTeacherData(teacherId, i) {
    ...
    this.currIndex = i;
    this.dataSurvey = [];
    this.repository.GetTeacherSurveyJanuary24(teacherId + "?" + this.user.userId)
      .then(response => {
  ...
  loadData(){
    ...
    if (this.cityId.length > 0 && this.subCityId.length > 0) {
      ...
      this.repository.GetTeacherSearchwithcityJanuary24(key+ "?" + this.user.userId)
        .then(response => {
          this.data = response as Array<ITeacherSearch>
        });
    }
