...
import { IElUsersModel, KinshipConcession, OnlineAdmissionUpdData, IAcademicInfo, OnlineApprovalStudent, AdmissionConcession } from '../../../../models/Admission/AdmissionForm';
...
export class ElUpdation extends Vue {
  ...
  admissionformid = '';
  concessscholarshiplist: Array<AdmissionConcession> = [];
  ...
  private referenceno = '';
  private classid = '';
  private list2: Array<KinshipConcession> = [];
  ...
  beforeModalOpen(event) {
    ...
    this.degreelist();
    this.studentlist = [];
    this.referenceno = event.params.REFFERENE.refferenceNo;
    this.admissionformid = event.params.REFFERENE.admissionFormId;
    this.campusprogramid = event.params.REFFERENE.campusProgramId;
    this.datas.push(event.params.REFFERENE);
    this.LoadKinshipConsesion();
  ...
  applyConcesson(admissionConcessionId, concessionId) {
    var key = admissionConcessionId + '?' + concessionId
    this.addrep.UpdateConcessKinStudent(key).then(r => {
      this.$store.dispatch(StoreTypes.updateStatusBar, {
        text: ' Kinship /Concession Inserted Successfully',
        title: 'Success',
  ...
  LoadKinshipConsesion() {
    this.concessscholarshiplist = []
    this.addrep.GetKinshipConcessScholar(this.admissionformid).then(r => {
      this.concessscholarshiplist = r as Array<AdmissionConcession>
    })
  }
  ...
  GetConcessionlist() {
    ...
    this.addrep.GetConcessionlist(this.campusprogramid).then(r => {
      var dta = r as Array<KinshipConcession>;
      this.list3 = [];

      dta.filter(e => e.concessionName.toLowerCase().indexOf('old') >= 0).forEach(element => {
  ...
  GetKInshiplist() {
    ...
    this.addrep.GetKinshipConcession(this.campusprogramid).then(r => {
      this.list2 = r as Array<KinshipConcession>
    })
  }
  ...
  save() {
    var response = confirm('Are You Sure to Approve')
    if (response) {

      this.addrep.InsertApprovalStudent(JSON.stringify(this.datas))
        .then(r => {
          this.$store.dispatch(StoreTypes.updateStatusBar, {
            text: 'Approval Done',
            title: 'Success',
