...
export class KinShipList extends Vue {
    ...
    private datas: Array<KinshipStudent> = [];
    ...
    private columns = [
        ...
        { key: 'type', caption: 'Kinship Type',sort: true },
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('kinshipstd' in this.user.claims) == true) {
                if (this.user.claims['kinshipstd'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['kinshipstd'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['kinshipstd'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['kinshipstd'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if (this.campusId.length > 0) {
            var key = this.sessionId+'?'+this.campusId;
            this.admissionRepo.GetKinshipStudent(key)
                .then(response => {
                    this.datas = (response as Array<KinshipStudent>)
    ...
    UpdateElRecord(option:any,option2:any,option3:any,option4:any,option5:any,option8:any,option9:any,option10:any,option11:any,option12:any,option13:any)
    {
      
        this.$modal.show('el-model',{ReffNO:option,ROLLTEACHID:option2,NAME:option3,FATHERNAME:option4,CAMPUSPROGRAMID:option8,ADMSIONFORMID:option5,TYPE:option9,CITY:option10,SESSION:option11,PROGRAM:option12,SECTION:option13});
