...
export class OverAllResultJanuary23 extends Vue {
  ...
  loadTotalSurvey() {
    ...
    this.survey = [];
    this.repository
      .TotalSurveyExJan23('b3228914-252e-4f90-b643-0047470059a6?5783ad6d-a901-4cfb-b160-1a8307752302')
  ...
  loadPosessionDataSurvey(id) {
    if (this.typeId == "Owned") {
      ...
      this.repository.SurveyStatisticsEx1LatestJanuary23(key).then((r) => {
        if (r) {
  ...
  loadCityDataSurvey(id) {
    ...
    if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
      ...
      this.repository.GetSurveyCityJanuary23WithBuilding(key).then((r) => {
      ...
      this.repository.GetSurveyCityJanuary23(key).then((r) => {
  ...
  loadbuildingdata() {
    if (this.subCityId.length > 0) {
      ...
      this.repository.BuildingSectionData(this.subCityId).then((r) => {
  ...
  loadTeachers(course) {
    ...
    if (this.cityId.length == 0 && this.subCityId.length == 0 ) {
      ...
      this.repository.TeacherRatingswithidJanuary2023(key).then((r) => {
...
this.repository.Jan23TeacherRatingswithidSpecificCityWithBuilding(key.trim()).then((r) => {
  this.teacherRatingList = r as Array<ITeacherRatingOverAllListwithid>;
  ...
  loadSectionData(id) {
    ...
    this.repository.GetTeacherGraphSectionWisewithtotalJanuary23Xx(key).then((r) => {
    ...
    this.repository.GetTeacherGraphSectionWisewithtotalJanuary23EXx(key).then((r) => {
      this.SectionData = r as Array<ITeacherRatingGraphEXSectionwithtotal>;
      this.data1 = true;
  ...
  refreshData() {
    ...
    this.repository.GetOverAllResultJanuary23(key).then((r) => {
  ...
  loadData() {
    ...
    ${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}?${this.buildingname}`;
          ...
          if (this.buildingname.length > 0 && this.buildingname != '' && this.building[0].isBuilding == true) {
            ...
            this.repository.GetOverAllResult1January23WithBuilding(key.trim()).then((r) => {
              this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
              this.loadQuestions();
              this.loadsummery();
            });
          ...
          this.showloader = true;
          this.repository.GetOverAllResult1withposissionJanuary23(key).then(r => {
            if (r) {
...
${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}?${this.buildingname}`;
    ...
    else
    {
      ...
      ${this.classId != "" ? this.classId : this.empty}?${this.whichModule}?${this.user.userId}`;
            this.showloader = true;
            this.repository.GetOverAllResult1January23(key.trim()).then((r) => {
              if (r) {
                this.surveyOverAllResultList = r as Array<ISurveyOverAllResultApril>;
                this.loadQuestions();
                this.loadsummery();
