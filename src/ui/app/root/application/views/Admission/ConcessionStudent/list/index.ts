...
import { ConcessModel } from '../concessapply';
...
@Component({
    ...
    template: require('./index.html'),
     components: {
         ...
         'concess-model':ConcessModel
     }
})

export class ConcessStdList extends Vue {
    ...
    validatePage() {
        if (this.user.roles.indexOf('admin') >= 0) {
            ...
            if (('concessstdd' in this.user.claims) == true) {
                if (this.user.claims['concessstdd'].indexOf('R') >= 0) {
                    this.canRead = true;
                }
                if (this.user.claims['concessstdd'].indexOf('C') >= 0) {
                    this.canAdd = true;
                }
                if (this.user.claims['concessstdd'].indexOf('U') >= 0) {
                    this.canEdit = true;
                }
                if (this.user.claims['concessstdd'].indexOf('D') >= 0) {
                    this.canDelete = true;
                }
            } else {
    ...
    refreshData() {
        ...
        if (this.campusId.length > 0) {
            var key = this.sessionId+'?'+this.campusId;
            this.admissionRepo.GetConcessStudent(key)
                .then(response => {
    ...
    ApplyConcession(option: any, option2: any) {

        this.$modal.show('concess', { CAMPUSPROGRAMID: option2, ADMSIONFORMID: option });
