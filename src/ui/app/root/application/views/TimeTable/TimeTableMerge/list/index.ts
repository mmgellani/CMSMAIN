...
export class TimeTableTimeTableMergeList extends Vue {
  ...
  private columns = [
    ...
    { key: "fromDate", caption: "StartTime" },
    { key: "toDate", caption: "EndTime" },
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("timetableMerge" in this.user.claims == true) {
        if (this.user.claims["timetableMerge"].indexOf("R") >= 0) {
          this.canRead = true;
        }
        if (this.user.claims["timetableMerge"].indexOf("C") >= 0) {
          this.canAdd = true;
        }
        if (this.user.claims["timetableMerge"].indexOf("U") >= 0) {
          this.canEdit = true;
        }
        if (this.user.claims["timetableMerge"].indexOf("D") >= 0) {
          this.canDelete = true;
        }
      } else {
  ...
  insertModel() {
    ...
    if (this.sessionid.length > 0 && this.classid.length > 0) {
      ...
      this.$modal.show("add-edit-model", { SESSIONID:this.sessionid,CLASSID:this.classid, IsNewRecord: true })
    }
    else {
  ...
  GetMergeTimetableData() {

    this.repository.GetMergeTimeTableData("Array[" + this.TimetableList.toString() + "]::uuid[]").then(
      r => {
        this.TimetableMergeList = r as Array<ITimeTableTimeTableMerge>
      })
  }
  insertModelBulk() {
  ...
  editModel(model: ITimeTableTimeTableMerge) {



    this.$modal.show("add-edit-model", { modelVM2: model,SESSIONID:this.sessionid,CLASSID:this.classid, IsNewRecord: false })
