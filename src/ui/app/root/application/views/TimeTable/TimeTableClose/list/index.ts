...
export interface ITimeTableClosed {
...
export class TimeTableClose extends Vue {
  ...
  private DataResponse: Array<ITimeTableClose> = [];
  private timetablearray:any = [];
  ...
  private columns = [
    ...
    { key: 'programName', caption: 'Program Name' },
    { key: 'shiftName', caption: 'Shift Name' }, 
  ...
  GetTimeTableCloseData(){
   ...
   if( this.sessionid.length>0 && this.campusid.length>0 && this.classid.length>0){
    ...
    this.classid ;
  this.repository
    .GetTimeTableCloseData(key)
    .then(response => {
      this.DataResponse = response as Array<ITimeTableClose>;

   });
  }
}
updatechek() {
  if (this.DataResponse.filter(e => e.isChecked == true).length > 0) {
...
//   this.isDisabled=false;
//    this.timetablearray=[];
//           this.DataResponse = this.DataResponse.filter(e => e.isChecked == true);
//           this.DataResponse.forEach(e =>
//             this.timetablearray.push(
//               {campusProgramId:e.campusProgramId,classId: this.classid}
//             )
//             );

//           var key = JSON.stringify(this.timetablearray)
//           debugger
          // this.repository.InsertTimeTableClosedData( key) 
          //         .then(response => {
          //      this.GetTimeTableCloseData();
          //            this.$store.dispatch(StoreTypes.updateStatusBar, {
          //                 text: 'TimeTable Closed SuccessFfully',
          //                title: '',
...
updatedata() {
  ...
  this.isDisabled=false;
          this.DataResponse = this.DataResponse.filter(e => e.isChecked == true);
          this.DataResponse.forEach(e => this.updMicrosoftPass(e));
          this.$store.dispatch(StoreTypes.updateStatusBar, {
              text: 'TimeTable Closed SuccessFfully',
              title: '',
              messageTypeId: PayloadMessageTypes.success
      ...
      updMicrosoftPass(message: ITimeTableClosed) {
  
          var key =this.sessionid+'?'+this.campusid+'?'+message.programId+'?'+message.campusProgramId + '?' +this.classid;
          this.repository.InsertTimeTableClosedData( key) 
          .then(response => {
       this.GetTimeTableCloseData();
       this.showmodel=false;
  ...
  getClass(day: string, slot: string) {
    var text = `<span style="font-size:14px; display:block; color:#57555d;">$course ($section)</span>
    <span style="font-size:10px; display:block;color:#6879fe;">[$room]</span>
    <span style="font-size:9px; display:block;color:#b0b0b1;">$staff</span>`;
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("timeTableClose" in this.user.claims == true) {
        if (this.user.claims["timeTableClose"].indexOf("R") >= 0) {
          this.canRead = true;
        }
        if (this.user.claims["timeTableClose"].indexOf("C") >= 0) {
          this.canAdd = true;
        }
        if (this.user.claims["timeTableClose"].indexOf("U") >= 0) {
          this.canEdit = true;
        }
        if (this.user.claims["timeTableClose"].indexOf("D") >= 0) {
          this.canDelete = true;
        }
      } else {
