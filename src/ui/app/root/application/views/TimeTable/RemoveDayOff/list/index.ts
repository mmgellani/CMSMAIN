...
import { TimeTableTimeTableAddEditt } from "../add-edit";
import { TimeTableTimeTableDeletee } from "../delete";
...
@Component({
  ...
  components: {
    "add-edit-model": TimeTableTimeTableAddEditt,
    "delete-model": TimeTableTimeTableDeletee,
...
export class RemoveDayOffBulkList extends Vue {
    ...
    this.TimeTableRepository.InsertBulkDayOff(res).then(
      r => {
        this.$store.dispatch(StoreTypes.updateStatusBar, {
          text: 'Bulk Day Off Entered SuccessFully',
          title: 'Success',
  ...
  saveModel2() {
    ...
    console.log(res)
    this.TimeTableRepository.RemoveBulkDayOff(res).then(
      r => {
        this.$store.dispatch(StoreTypes.updateStatusBar, {
  ...
  validatePage() {
    if (this.user.roles.indexOf("admin") >= 0) {
      ...
      if ("RemoveDayOffBulkList" in this.user.claims == true) {
        if (this.user.claims["RemoveDayOffBulkList"].indexOf("R") >= 0) {
          this.canRead = true;
        }
        if (this.user.claims["RemoveDayOffBulkList"].indexOf("C") >= 0) {
          this.canAdd = true;
        }
        if (this.user.claims["RemoveDayOffBulkList"].indexOf("U") >= 0) {
          this.canEdit = true;
        }
        if (this.user.claims["RemoveDayOffBulkList"].indexOf("D") >= 0) {
          this.canDelete = true;
        }
      } else {
