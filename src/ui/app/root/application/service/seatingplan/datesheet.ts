...
const BASE_URL = GlobalConfig.uri.services + 'SeatingPlanDateSheet/';

export class SeatingPlanDateSheetService extends StoreService {
    ...
    public AddBulk(data: any) {
        return this.exec<any>(Axios.post(BASE_URL + 'AddBulk', { ProvidedString: data }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
    ...
    public GetAllDateSheetVM() {
        return this.exec<any>(Axios.get(BASE_URL + 'GetAllDateSheetVM'))
                    .then((value) => this.processPayload(value))
                    .catch((error) => console.error(error));
    }
