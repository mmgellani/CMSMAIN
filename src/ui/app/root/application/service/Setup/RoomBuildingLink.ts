...
import { ISetupRoomBuildingLink } from '../../models';
import { Store } from 'vuex';
import { StoreService } from '../../../../store';

const BASE_URL = GlobalConfig.uri.services + 'SetupRoomBuildingLink/';

export class SetupRoomBuildingLinkService extends StoreService {
    ...
    public GetAllRoomBuildingLink(predicate:string) {
        
        return this.exec<any>(Axios.post(BASE_URL + 'GetAllRoomBuildingLink', { ProvidedString: predicate }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
    ...
    public GetRoomfromSection(predicate: string) {
        return this.exec<any>(Axios.post(BASE_URL + 'GetRoomfromSection', { ProvidedString: predicate }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
    // public GetRoomforedit(predicate: string) {
    //     return this.exec<any>(Axios.post(BASE_URL + 'GetRoomforedit', { ProvidedString: predicate }))
    //         .then((value) => this.processPayload(value))
    //         .catch((error) => console.error(error));
    // }
