...
import { IProgramFeeAdjustment } from '../../models';

const BASE_URL = GlobalConfig.uri.services + 'ProgramFeeAdjustment/';

export class ProgramFeeAdjustmentService extends StoreService {
    ...
    public GetAllVMExDel(key:string) {
        return this.exec<any>(Axios.post(BASE_URL + 'GetAllVMExDel', { ProvidedString: key }))
                    .then((value) => this.processPayload(value))
                    .catch((error) => console.error(error));
    }
    public GetAllAsync() {
