...
import { IFeeCampusChallanNoteLink } from '../../models';

const BASE_URL = GlobalConfig.uri.services + 'FeeCampusChallanNoteLink/';

export class FeeCampusChallanNoteLinkService extends StoreService {
    ...
    public GetDataVM(predicate: string) {
        return this.exec<any>(Axios.post(BASE_URL + 'GetDataVM', { ProvidedString: predicate }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
