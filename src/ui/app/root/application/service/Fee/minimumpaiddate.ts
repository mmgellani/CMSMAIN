...
import { IMinimumPaidDate } from '../../models';

const BASE_URL = GlobalConfig.uri.services + 'MinimumPaidDate/';

export class MinimumPaidDateService extends StoreService {
    ...
    public GetFindByChallanType(predicate: string) {
        return this.exec<any>(Axios.post(BASE_URL + 'GetFindByChallanType', { ProvidedString: predicate }))
                    .then((value) => this.processPayload(value))
                    .catch((error) => console.error(error));
    }
