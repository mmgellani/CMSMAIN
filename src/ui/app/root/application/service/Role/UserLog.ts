...
const BASE_URL = GlobalConfig.uri.services + 'UserLog/';

export class RoleUserLogService extends StoreService {
    ...
    public FindBy(predicate: string) {
        return this.exec<any>(Axios.post(BASE_URL + 'FindBy', { ProvidedString: predicate }))
                    .then((value) => this.processPayload(value))
                    .catch((error) => console.error(error));
    }
