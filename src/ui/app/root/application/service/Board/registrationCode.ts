...
const BASE_URL = GlobalConfig.uri.services + 'RegistrationCode/';

export class RegistrationCodeService extends StoreService {
    ...
    public GetFindByPg(predicate: string) {
        
        return this.exec<any>(Axios.post(BASE_URL + 'GetFindByPg', { ProvidedString: predicate }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
