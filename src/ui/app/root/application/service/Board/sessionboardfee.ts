...
const BASE_URL = GlobalConfig.uri.services + 'SessionBoardFee/';

export class SessionBoardFeeService extends StoreService {
    ...
    public FindPk(predicate: string) {
        return this.exec<any>(Axios.post(BASE_URL + 'FindPk', { ProvidedString: predicate }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
