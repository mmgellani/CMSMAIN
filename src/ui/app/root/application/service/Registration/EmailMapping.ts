...
const BASE_URL = GlobalConfig.uri.services + 'CampusEmailMapping/';

export class CampusEmailMappingService extends StoreService {
    ...
    public GetEmailMappinglist(predicate: string) {
        return this.exec<any>(Axios.post(BASE_URL + 'GetEmailMappinglist', { ProvidedString: predicate }))
            .then((value) => this.processPayload(value))
            .catch((error) => console.error(error));
    }
