...
import ExAuthService from '../../services/AuthService';


type ValidateLogin = { data: ICredential; validationGroup: string[] };
let customValidation: ValidationRuleset<ValidateLogin> = {
    data: {
        username: {
            email,
            required
...
@Component({
    name: 'Login',
    template: require('./loginadd.html'),
...
export class Login extends Vue {
    private authService = new ExAuthService();
    private auth: AuthenticationService = null;
    private data: ICredential = {
        username: "",
        password: ""
    };
    ...
    created() {
        ...
        this.clientWidth = (25 * window.innerWidth) / 100;
        ...
        if (errorCode) {
            ...
            onAuthError(errorCode.toString(), this.$route.query['errorDesc'].toString().replace('/', ''));
        }
    ...
    changeBodyClass() {
        // Clean body for the login page layout
        document.body.className = 'ems-login-page';
    }
    ...
    public get returnUrl(): string {
        ...
        return returnUrl ? returnUrl.toString().replace(window.location.origin, '') : null;
    }

    public get hasExternalProviders(): boolean {
        return this.externalProviders && this.externalProviders.length < 2;
    }
    get allowSubmit() {
