...
function createMount() {
    ...
    const config = webpackBase(outputPath, srcPath, '/', false);
...
function createAreas() {
    ...
    config.optimization = {
        // Parallel, cached minification (single-threaded Terser is a major bottleneck).
        minimizer: [
            new TerserPlugin({
                parallel: true,
                cache: true,
                terserOptions: {
                    // Skip expensive comment extraction work.
    ...
    const tsLoader = {
        ...
        use: [
            'babel-loader',
            {
                loader: 'ts-loader',
                options: {
                    // Skip type-checking during the build (the slowest part for ~1400 files).
                    // Type errors should be caught by the editor / a separate `tsc --noEmit` step.
                    transpileOnly: true,
                    // Only compile files actually referenced by the bundle, not the whole project.
                    onlyCompileBundledFiles: true,
                    // Speeds up emit; we don't rely on .d.ts output for a web bundle.
                    compilerOptions: {
                        declaration: false
                    }
                }
